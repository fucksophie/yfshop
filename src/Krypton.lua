...
function makeaddressbyte(byte)
  local bb = 48 + math.floor(byte / 7)
  return string.char(bb + 39 > 122 and 101 or bb > 57 and bb + 39 or bb)
end
...
function Krypton:post(endpoint, data)
    ...
    local dd = textutils.unserializeJSON(body)
    if not dd.ok then
        if (dd.error ~= "invalid_parameter") then
            error(dd.error, 3)
        else
            error({dd.error, dd.parameter}, 3)
        end
    end
    return dd
end
