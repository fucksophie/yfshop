...
function krist.eventListener()
    while true do
        ...
        if transactionEvent.transaction.from ~= krist.krypton:makev2address(krist.settings.privateKey) then
            ...
            if string.find(krist.settings.address, ".kro") then
                ...
                if transactionEvent.transaction.metadata then
                    ...
                    if not whitelisted then
                        ...
                        if type(result) == "number" then
                            krist.ws:makeTransaction(ret, result,
                                "message=You spent too much! Overflowed " .. result .. "KST.")
                        elseif type(result) == "boolean" then
                            if result then
                                krist.ws:makeTransaction(ret, transactionEvent.transaction.value,
                                    "message=Return (of item " .. item .. ") to address " .. ret .. "")
                            end -- if this is false we do nothing
                        end
                    end
                else
                    krist.ws:makeTransaction(transactionEvent.transaction.from, transactionEvent.transaction.value,
                        ret .. " please include a m-name!")
                end
            end
        end
    end
